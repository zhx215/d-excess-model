...
reevap <- function(p0,temp0,rh0,D,d18Ov,d2Hv) {
  # equilibrium fractionation from vapor to liquid (or ice) phase (d18O) (>1) (Majoube, 1971)
  a18_P_lv <- function(temp) {
    if (temp >= 273.15) {exp(1137/temp^2-0.4156/temp-0.00207)}
      ...
      iv*(273.15-temp)/20+lv*(temp-253.15)/20 # two component mass balance, like Nusbaumer et al., 2017
    } 
  }
  
  # equilibrium fractionation from vapor to liquid (or ice) phase (d2H) (>1) (Majoube, 1971)
  ...
  a18_P_k <- function(temp){
    ...
    else {Si/(a18_P_iv*diff16_18*(Si-1)+1)*(273.15-temp)/20+(temp-253.15)/20*1} # two component mass balance, like Nusbaumer et al., 2017
  }
  
  # kinetic fractionation due to supersaturation over ice, d2H (Jouzel and Merlivat, 1984)
  a2_P_k <- function(temp){
    Si <- 1-0.004*(temp-273.15) # lambda = 0.004, Risi et al., 2010
    diff1_2 <- 1.0251 # (D/D') for 1H/2H
    a2_P_iv <- exp(16289/temp^2-0.0945)
    if (temp >= 273.15) {1}
    else if (temp < 253.15) {Si/(a2_P_iv*diff1_2*(Si-1)+1)}
    else {Si/(a2_P_iv*diff1_2*(Si-1)+1)*(273.15-temp)/20+(temp-253.15)/20*1} # two component mass balance, like Nusbaumer et al., 2017
  }
  ...
  rv18 <- (d18Ov/1000+1)*rstd18 # get r value for 18O/16O in vapor, it can be modified to study equilibration
  rv2 <- (d2Hv/1000+1)*rstd2 # get r value for 2H/1H in vapor, it can be modified to study equilibration
  rp18_p <- rv18*a18_P_lv(td_p)*a18_P_k(td_p) # get r value for 18O/16O in raindrop, initial value
  rp2_p <- rv2*a2_P_lv(td_p)*a2_P_k(td_p) # get r value for 2H/1H in raindrop, initial value
  ...
  repeat{
    ...
    if (t_p[i]<=273.15){ # if surrounding air is frozen, no evaporation
      ...
      e_rate_2 <- 2*pi*(D_p[i]/1000)*fv_p[i]*diff_p[i]/Rw*(fv_p2[i]/fv_p[i]/diff1_2)^0.58*
        (rh_p[i]*rv2*get_es(t_p[i])/t_p[i]-rp2_p[i]/a2_P_lv(td_p[i])*get_es(td_p[i])/td_p[i])
    }
