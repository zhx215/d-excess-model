...
source('lcl.R')
reevap <- function(p0,temp0,rh0,D,d18Ov,d2Hv) { # inputs: surface pressure, surface temperature, surface RH, initial raindrop diameter, vapor d18O, vapor d2H 
  ...
  a18_P_lv <- function(temp) {
    if (temp >= 273.15) {exp(1137/temp^2-0.4156/temp-0.00207)}
      ...
      iv*(273.15-temp)/20+lv*(temp-253.15)/20 # aquilot mixing, similar to equation (21) in Nusbaumer et al., 2017
    } 
  }
  ...
  a18_P_k <- function(temp){
    ...
    else {Si/(a18_P_iv*diff16_18*(Si-1)+1)*(273.15-temp)/20+(temp-253.15)/20*1} # aquilot mixing, similar to equation (21) in Nusbaumer et al., 2017
  }
  ...
  aD_P_k <- function(temp){
    ...
    else {Si/(aD_P_iv*diff1_2*(Si-1)+1)*(273.15-temp)/20+(temp-253.15)/20*1} # aquilot mixing, similar to equation (21) in Nusbaumer et al., 2017
  }
